...
const removeProductFromCart = async (productId) => {
  try {
    const response = await axios.delete(`${baseURL}/cart/product`, {
      data: { productId },
    });
...
const updateProductQuantityInCart = async (productId, quantity) => {
  try {
    const response = await axios.put(`${baseURL}/cart/product`, {
      productId,
      quantity,
    });
...
const chaosMonkeyTests = async () => {
  const handleRequest = async (requestFunc, ...args) => {
    try {
      ...
      console.error(
        `Error in ${requestFunc.name}:`,
        error.response ? error.response.data : error.message,
      );
    }
  };

  await handleRequest(axios.post, `${baseURL}/products`, {
    name: "",
    price: -100,
    stock: -10,
  });
  await handleRequest(axios.post, `${baseURL}/cart/product`, {
    productId: "nonexistentproduct",
    quantity: 1,
  });
  await handleRequest(axios.delete, `${baseURL}/cart/product`, {
    data: { productId: "nonexistentproduct" },
  });
  await handleRequest(axios.put, `${baseURL}/cart/product`, {
    productId: "nonexistentproduct",
    quantity: 5,
  });
  await handleRequest(applyDiscount, "INVALIDCODE");
  await handleRequest(axios.post, `${baseURL}/users/login`, {
    username: "invaliduser",
    password: "invalidpassword",
  });
  await handleRequest(axios.post, `${baseURL}/cart`);
  await handleRequest(axios.post, `${baseURL}/cart/product`, {
    productId,
    quantity: 1,
  });
  await handleRequest(axios.get, `${baseURL}/cart/summary`);
};

...
const registerUser = async () => {
  try {
    ...
    if (error.response && error.response.data.error.includes('duplicate key error')) {
      console.error('User already exists, proceeding to login');
    } else {
...
const chaosMonkeyTests = async () => {
  try {
    ...
    console.error('Chaos monkey test error (invalid product creation):', error.response ? error.response.data : error.message);
  }
  try {
    await axios.post(`${baseURL}/cart/product`, { productId: 'nonexistentproduct', quantity: 1 });
  } catch (error) {
    console.error('Chaos monkey test error (nonexistent product add to cart):', error.response ? error.response.data : error.message);
  }
  try {
    await axios.delete(`${baseURL}/cart/product`, { data: { productId: 'nonexistentproduct' } });
  } catch (error) {
    console.error('Chaos monkey test error (nonexistent product remove from cart):', error.response ? error.response.data : error.message);
  }
  try {
    await axios.put(`${baseURL}/cart/product`, { productId: 'nonexistentproduct', quantity: 5 });
  } catch (error) {
    console.error('Chaos monkey test error (nonexistent product update in cart):', error.response ? error.response.data : error.message);
  }
  try {
    await applyDiscount('INVALIDCODE');
  } catch (error) {
    console.error('Chaos monkey test error (invalid discount code):', error.response ? error.response.data : error.message);
  }
  try {
    await axios.post(`${baseURL}/users/login`, { username: 'invaliduser', password: 'invalidpassword' });
  } catch (error) {
    console.error('Chaos monkey test error (invalid login):', error.response ? error.response.data : error.message);
  }
};
